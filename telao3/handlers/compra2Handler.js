...
async function handleCompra2(sock, msg) {
  ...
  try {
    // ✅ Extrai quantidade de MBs da mensagem
    ...
    // ✅ Pegando o usuário mencionado na mensagem respondida
    const mentionedJid = msg.message?.extendedTextMessage?.contextInfo?.participant;
    if (!mentionedJid) {
      return await sock.sendMessage(from, { text: '❌ Você precisa *responder* a mensagem do comprador para registrar a compra!' });
    }
